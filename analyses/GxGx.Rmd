...
## Behavioral measures
### Measure 1 - hop distance

The simplest measurement is the distance moved between frames. For each recording, behavior was filmed over two minutes, with the plate being dropped in the middle of the recording. So *a priori* we can divide a recording into three sections: before the drop, during the drop, and after the drop.  Let's see how 'hop distance' changes throughout a recording.
...
Here is the average proportion of time spent moving over the recording.
...
As expected, the patterns are very similar to those observed with just 'hop distance'. The auto tracker seems to miss some movements at the start of the recording, which probably has to do with how quickly the video's 'background' is established by the algorithm and then subtracted out to detect movement. It also detects less movement overall, which is probably due to the higher frame rate - more movement can happen in 2 seconds (manual frame rate) than in 1/8 of a second (auto frame rate).

Turning to the second component of 'hop distance' - when a copepod is moving, how far it moves. I'll call this the velocity, because it is the distance moved in a set time frame, i.e. a rate. The next two plots show how velocity changes over the recording. There is a clear difference between auto and manually tracked data.
```{r}
...
ggplot(filter(bdr_avg3, group != 'drop'),
  ...
  labs(title = 'auto tracker', y = 'mean velocity when moving + CI')
...
ggplot(filter(bd_avg3, group != 'drop'),
  ...
  geom_path(color = 'black') +
  labs(title = 'manual tracking', y = 'mean velocity when moving + CI')
```

Right after the drop, copepods seems to exhibit large movements in the auto dataset, which is tempting to interpet as escape movements, but the opposite is seen in the manual tracking data. So, again it looks like there some artifacts in the auto-tracking dataset, probably due to the background being shifted when a plate is dropped. However, in both cases, it is worth noting that when we do not focus on the time right after the drop, it doesn't look like the drop has a profound effect on this variable. For most of the time after the drop, copepod movements look similar to before the drop.

Next we check whether the number of movements (proportion of time moving) is correlated to the magnitude of movements (velocity). When we plot these components, we see very high values in copepods that hardly moved. This is because a single, large perhaps erroneously recorded movement can determine the average velocity over the recording when few movements are detected.
...
When we zoom in to exclude the copepods with a few, high magnitude movements, then we see that there is not a strong correlation between these variables. The residual variation suggests that in some recordings copepods are more slow and steady movers while in others they are more fast and jerky. 
...
Again, there are idiosyncratic differences between the datasets, like in the auto-tracking dataset the combinations of high velocities and a low proportion of time moving, which is presumably an artifact. The qualitative patterns appear similar in both datasets. The infecteds clearly spend less time moving than uninfecteds (shifted on the x-axis), but when they are moving, velocities are comparable (minimal shift along the y-axis). Additionally, the responses to the drop are subtle, suggesting copepods do not substantially change their behavior in response to the 'shock'. Indeed, the time-series (plots above)[] suggest that on average they recover to pre-drop levels rather quickly.

Here's another way to visualize treatment and drop effects in the different behavior variables. Until now, we have looked averages across recordings and groups, ignoring how copepods respond to a drop within a recording. The next plots show how individuals respond to the drop, first for proportion of time moving. The overall average response is the bold line.
...
ggplot(bd_avg4, aes(x = drop, y = prop_moving, color = trt)) + 
  ...
  facet_grid(~trt) +
  labs(title = 'manual tracker')
```
The qualitative pattern is very similar in the two datasets - infecteds spend less time moving in general and all groups reduce activity after the drop. The raw values are higher in the manual data, but this makes sense given it's lower frame rate, i.e. more movement is possible within 2 seconds than an 1/8 of a second.

Making the same plot for our second response variable, copepod velocity when moving...
...
The pattern is quite similar in both datasets. Activity decreases in response to the drop, particularly in the uninfected groups, because they were more active to begin with. Infected copepods are less active regardless of whether they are observed before or after the drop.

ARGUE FOR USING THIS ONE COMPOSITE MEAUSRE! One problem is bizarre dist.
