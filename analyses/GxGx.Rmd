...
Even with a reduced frame rate, there is still substantial pseudoreplication in the data - within a recording, observations are separated by half a second and are thus conceivably highly-correlated. In previous analyses with this kind of data, we fit mixed models to account for this (i.e. recording was a random effect - see e.g. this [study](LINK!!!)). However, this might be insufficient, because it assumes the same covariance between all observations within a recording. In other words, it does not differentiate between two observations in the same recording that are 2 seconds apart or that are 20 seconds apart. A solution would be to fit some kind of autoregressive function that models how similarity decays over time within a recording. But this would be difficult. When we look at individual recordings like a time-series, we see peaks (the copepod is hopping) and valleys (the copepod is stationary). There is not the obvious temporal autocorrelation (i.e. time points near each other more similar than time points further away) that we can see when averaging over all recordings (see [plot above](LINK!!!)).
...
Thus, trying to model this data at the find level of each frame seems to be quite complex. The old-fashioned approach of averaging out pseudoreplication might be preferable (and easier for readers to understand).
...
Obviously, there is still some variation within these groups, particularly the 'shocked' group.
