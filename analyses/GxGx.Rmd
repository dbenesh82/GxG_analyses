...
This clearly shows that the infecteds are less active overall than the uninfecteds. The difference may be slightly smaller after the drop, because uninfecteds decrease their activity more substantially (or the infecteds just can't decrease much more). The only part of the time-series where there is not a treatment difference is right after the drop. But the difference in behavior is noticeable within a few seconds again, so again it does not seem necessary to consider this separately.

# Summary

Which dataset? Next analysis - defining manipulation (absolute value vs relative to controls) and GxG. 
